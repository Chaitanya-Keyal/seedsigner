...
def test_render_lists_changes_and_escapes_untrusted_msgid():
    malicious = '<script>alert(1)</script> `back` </details>'
    m = base_manifest(
        source_strings={
            "added": [{"msgid": malicious, "msgctxt": None, "plural": False}],
            "removed": [], "changed": [],
    ...
    assert "changes the translatable source strings" in body
